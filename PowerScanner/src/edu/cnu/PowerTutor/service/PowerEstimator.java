...
import edu.cnu.PowerTutor.widget.PowerWidget;
	...
	private void openLog(boolean init) {
		...
		try {
			...
			if (init && prefs.getBoolean("sendPermission", true)
				...
				/*
				 * There is data to send. Make sure that gets going in the
				 * sending process before we write over any old logs.
				 */
	...
	public void run() {
		...
		for (long iter = -1; !Thread.interrupted();) {
			...
			for (int i = 0; i < components; i++) {
				...
				// �닿납�먯꽌 �ㅼ젣 Power媛�怨꾩궛�섏뼱 吏꾨떎. profile�뺣낫��powerData����옣�섏뼱
				// �덈떎.
				// powerData���꾨줈�뚯씪 �뺣낫媛��ㅼ뼱�덈떎. �먰븳 �ъ궗�⑹씠 媛�뒫��媛앹껜 援ъ“濡��섏뼱
				// �덈떎.
				// powerData�덉뿉 cachedPower�쇨퀬�댁꽌 profile����븳 power
				// consumption�뺣낫瑜�湲곗엯�쒕떎.
				...
				for (int j = 0; j < uidPower.size(); j++) {
					...
					int power = (int) powerFunctions.get(i)
							.calculate(powerData);
			...
			synchronized (fileWriteLock) {
				synchronized (uidAppIds) {
					for (int i = 0; i < components; i++) {
						...
						SparseArray<PowerData> uidPower = data
								.getUidPowerData();
						for (int j = 0; j < uidPower.size(); j++) {
							int uid = uidPower.keyAt(j);
			...
			if (iter % 15 == 14) {
				...
				context.updateNotification(
						(int) Math.min(8,
								1 + 8 * avgPower / phoneConstants.maxPower()),
						avgPower);
			}
			...
			synchronized (fileWriteLock) {
				...
				if (iter % 15 == 0 && prefs.getBoolean("sendPermission", true)) {
					/*
					 * Allow for LogUploader to decide if the log needs to be
					 * uploaded and begin uploading if it decides it's
					 * necessary.
					 */
		...
		/*
		 * Close the logstream so that everything gets flushed and written to
		 * file before we have to quit.
		 */
		synchronized (fileWriteLock) {
			if (logStream != null)
				try {
	...
	public long getUidExtra(String name, int uid) {
		if ("OLEDSCORE".equals(name)) {
			...
			double result = oledScoreHistory
					.getTotal(uid, Counter.WINDOW_TOTAL) / 1000.0;
			result /= entries;
			PhoneConstants phoneConstants = PhoneSelector.getConstants(context);
