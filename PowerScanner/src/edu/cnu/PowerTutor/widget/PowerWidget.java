...
package edu.cnu.PowerTutor.widget;
...
public class PowerWidget extends AppWidgetProvider {
	...
	private static final int[] text_ids = { R.id.text_minute, R.id.text_hour,
			R.id.text_day, };
	...
	public static void updateWidgetDone(Context context) {
		...
		Intent notificationIntent = new Intent(context, PM_MainActivity.class);
			...
			try {
				...
				if (val != null) {
					...
					for (int i = 0; i < text_ids.length; i++) {
						...
						views.setTextViewText(text_ids[i],
								dataSource.getValue(p));
					}
					ok = true;
				} else {
