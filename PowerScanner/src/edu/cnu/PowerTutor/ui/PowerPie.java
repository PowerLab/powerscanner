...
public class PowerPie extends Activity {
	...
	public static final int[] COLORS = new int[] { Color.BLUE, Color.GREEN,
			Color.MAGENTA, Color.YELLOW, Color.RED, Color.LTGRAY, Color.DKGRAY,
			Color.CYAN };
	...
	public void refreshView() {
		...
		/*
		 * The collector is responsible for periodically updating the screen
		 * with new energy usage information for the current uid.
		 */
			...
			public void run() {
				try {
					...
					for (int i = 0; i < totals.length; i++) {
						totals[i] = totals[i]
								* PowerEstimator.ITERATION_INTERVAL / 1000;
						sumTotal += totals[i];
					}
					int index = 0;
					if (sumTotal < 1e-7) {
					...
					Log.w(TAG,
							"Failed to contact power tutor profiling service");
				}
				if (handler != null) {
