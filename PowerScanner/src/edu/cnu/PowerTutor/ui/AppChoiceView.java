...
import edu.umich.PowerTutor.util.CompareFunc;

/**
 * this class is seleting package name so return sending server activity
 */
public class AppChoiceView extends ListActivity implements OnTouchListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setListAdapter(new ProcessListAdapter(this, appList2)); // 커스터마이즈한 어뎁터를
																// 달아준다.
		/*
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		SeletedPackageName = mPackageManager.getNameForUid(appList2
				.get(position).uid);
	 ...
	 */
	public void showMessageDialog(final CharSequence message,
		...
		runOnUiThread(new Runnable() {
			public void run() {
				...
				AlertDialog.Builder builder = new AlertDialog.Builder(
						AppChoiceView.this);
				builder.setMessage(message + " " + SeletedUid + " "
						+ SeletedPackageName);
				...
				builder.setNegativeButton("cancel",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								// TODO Auto-generated method stub
								// 취소 했다는 메시지를 출력한다.
								Toast.makeText(AppChoiceView.this, "cancel",
	...
	private class ProcessListAdapter extends BaseAdapter {
		...
		public ProcessListAdapter(Context context,
				List<ActivityManager.RunningAppProcessInfo> arList) {
			mContext = context;
			mList = arList;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// 현재 위치의 아이템을 얻어온다.
			ActivityManager.RunningAppProcessInfo info = (ActivityManager.RunningAppProcessInfo) mList
					.get(position);

			// 이름에 ':'문자를 포함하고 있다면 :전까지의 내용을 PackageName으로 정의한다. 이렇게 해야 정상적으로
			// icon과 label을 추출 할 수 있다.
			if (info.processName.contains(":")) {
				PackageName = info.processName.substring(0,
						info.processName.indexOf(":"));
			} else
			...
			if (appPackageInfo == null && info.uid > 0) {
				...
				String[] subPackageName = mPackageManager
						.getPackagesForUid(info.uid);
				...
				if (subPackageName != null) {
					for (int PackagePtr = 0; PackagePtr < subPackageName.length; PackagePtr++) {
						...
						try {
							appPackageInfo = mPackageManager.getPackageInfo(
			...
			LabelName = appPackageInfo.applicationInfo.loadLabel(
					mPackageManager).toString();
			Icon = resizeImage(appPackageInfo.applicationInfo
					.loadIcon(mPackageManager));

			/*
			 * if (convertView == null) {
			 */
			sv = new ProcessDetailView(mContext, Icon, info.pid, info.uid,
					LabelName, position);

			/*
			 * } else { sv = (ProcessDetailView) convertView; sv.setView(Icon,
			 * info.pid,info.uid, LabelName, position);
			 * 
			 * 
			 * }
			 */
	...
	private class ProcessDetailView extends TableLayout {
		...
		public ProcessDetailView(Context context, Drawable Icon, int PID,
				int UID, String Name, int position) {
			super(context);
	...
	private Drawable resizeImage(Drawable Icon) {
		...
		if (CompareFunc.getScreenSize() == 2) {
			Bitmap BitmapOrg = Bitmap.createBitmap(60, 60,
			...
			Bitmap BitmapOrg = Bitmap.createBitmap(10, 10,
					Bitmap.Config.ARGB_8888);
			Canvas BitmapCanvas = new Canvas(BitmapOrg);
			...
			Bitmap BitmapOrg = Bitmap.createBitmap(22, 22,
					Bitmap.Config.ARGB_8888);
			Canvas BitmapCanvas = new Canvas(BitmapOrg);
